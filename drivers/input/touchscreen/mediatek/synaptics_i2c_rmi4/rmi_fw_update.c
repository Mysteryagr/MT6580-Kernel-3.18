...
static int synaptics_get_firmware_verno(char *chr);
...
static int fwu_check_version(void)
{
	...
	unsigned char firmwareVerno;
	unsigned char firmwareICVerno;
	...
	TPD_DMESG(".img config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
       ...
       firmwareICVerno = synaptics_get_firmware_verno(config_id);
       firmwareVerno = synaptics_get_firmware_verno(fwu->config_data);
	if((firmwareICVerno < firmwareVerno) ||ftm_force_update)
       {
             TPD_DMESG("firmwareICVerno is lower than firmwareVerno or fwu->force_update is true!\n");
	      TPD_DMESG("Start update image is accessed!\n");
             return 0;
       } else {
             TPD_DMESG("firmwareICVerno is greater than firmwareVerno and fwu->force_update is false!\n");
	      TPD_DMESG("Start update image is not accessed!\n");
             return -1;
       }
	#if 0
...
//			return -EINVAL;
	...
	parse_header(&header, fw_image);
       TPD_DMESG("%s:fw_image[%d] = %x\n",__func__,FW_IMAGE_OFFSET + header.image_size,fw_image[FW_IMAGE_OFFSET + header.image_size]);
	if (header.image_size)
		fwu->firmware_data = fw_image + FW_IMAGE_OFFSET;
	if (header.config_size)
		fwu->config_data = fw_image + FW_IMAGE_OFFSET + header.image_size;
	TPD_DMESG("%s:.img config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",__func__,
		  fwu->config_data[0],
		  fwu->config_data[1], fwu->config_data[2], fwu->config_data[3]);
...
{
        int ret,reth,retl;
	 int configID = be_to_uint(chr);
        ret = reth = retl =0;
        reth = (configID&0xFF00)>>8; 
        retl = configID&0xFF;
        reth = reth - 0x30;
        retl = retl - 0x30;
        ret = reth*10 + retl;       
        printk("%s:gconfig_id=%x,ret=%d",__func__,configID,ret);
        //return gconfig_id;
        return ret;
}
static void synaptics_read_fw_version(void)
