...
#define CTP_BAK_REF_PATH                "/sdcard/syn"
/*
#define RAW_HEX
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	retval = sstrtoul(buf, 10, &setting);
             TP_DBG("%s: line = %d buf = %lx\n",__func__,__LINE__,setting);
	if (retval)
		return retval;

	if (!is_report_type_valid((enum f54_report_types)setting)) {

		TP_DBG("%s: Report type not supported by driver\n",__func__);
		return -EINVAL;
	}
	...
	if (f54->status != STATUS_BUSY) {
		...
		if (retval < 0) {
			...
			TP_DBG("%s: Failed to write data register\n",__func__);
			return retval;
		}
		return count;
	} else {
		TP_DBG("%s: Previous get report still ongoing\n",__func__);
		mutex_unlock(&f54->status_mutex);
		return -EINVAL;
	}
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = f54->fn_ptr->read(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		TP_DBG("%s: Failed to read data registers\n",__func__);
		return retval;
	}
...
990,1838,956,1774,949,1762,951,1767,950,1765,950,1765,950,1764,947,1759,945,1754,949,1763,965,1793,980,1821,
1267,2353,1324,2460,1313,2438,1289,2393,1292,2400,1293,2401,1281,2379,1278,2374,1281,2378,1299,2413,1317,2446,1389,2579,
1241,2304,1235,2294,1228,2281,1228,2280,1229,2282,1222,2270,1214,2255,1207,2242,1204,2237,1201,2230,1207,2241,1190,2210,
1031,1914,1099,2041,1096,2035,1092,2027,1088,2022,1087,2019,1081,2007,1077,2001,1075,1997,1075,1997,1080,2005,1089,2023,
1054,1957,1047,1945,1044,1938,1046,1942,1040,1932,1037,1927,1029,1912,1024,1902,1024,1902,1019,1893,1019,1893,986,1831,
973,1808,1053,1956,1055,1960,1052,1954,1047,1944,1044,1940,1038,1928,1029,1912,1031,1915,1026,1906,1028,1909,1036,1923,
1055,1960,1036,1923,1037,1927,1036,1924,1030,1913,1027,1907,1020,1894,1014,1883,1012,1879,1006,1869,1006,1868,941,1747,
966,1793,1036,1924,1036,1925,1037,1926,1029,1911,1027,1907,1018,1890,1012,1879,1009,1875,1005,1866,1004,1865,992,1842,
1045,1940,1033,1918,1034,1921,1033,1919,1026,1905,1024,1902,1018,1890,1010,1876,1010,1875,1003,1862,1001,1858,924,1715,
899,1669,1005,1866,1010,1875,1012,1879,1004,1864,999,1856,990,1838,982,1824,979,1818,975,1811,974,1808,987,1833,
961,1785,929,1725,930,1727,930,1727,921,1710,917,1702,910,1689,900,1672,900,1671,892,1657,889,1651,840,1561,
836,1553,904,1679,908,1687,910,1690,903,1676,900,1671,891,1654,882,1637,882,1638,876,1627,872,1620,898,1667,
882,1639,849,1576,850,1579,854,1586,845,1569,841,1563,831,1544,825,1532,822,1527,817,1518,814,1511,768,1427,
774,1437,812,1509,815,1513,819,1521,811,1507,808,1500,799,1484,793,1473,791,1469,786,1459,785,1457,818,1518,
833,1548,787,1462,788,1464,790,1467,783,1455,778,1445,769,1428,765,1421,762,1414,758,1408,756,1403,726,1348,
765,1420,792,1471,795,1476,799,1484,791,1469,786,1459,779,1447,774,1437,770,1430,766,1423,768,1426,812,1509,
847,1574,782,1452,783,1454,787,1462,778,1445,775,1440,768,1426,761,1413,758,1408,754,1401,753,1399,732,1360,
850,1579,879,1632,878,1631,881,1636,877,1628,871,1617,859,1595,857,1591,851,1581,845,1569,847,1574,915,1699,
1011,1877,862,1601,866,1608,873,1621,861,1599,858,1593,851,1580,842,1563,836,1553,836,1553,829,1540,813,1510,
634,1178,661,1228,667,1238,670,1245,665,1234,662,1229,655,1216,649,1206,649,1205,643,1194,643,1194,793,1473,};


/*
static ssize_t synaptics_rmi4_f54_alpha_test(struct file *data_file,
		struct kobject *kobj, struct bin_attribute *attributes,
		char *buf, loff_t pos, size_t count)
*/
static ssize_t synaptics_rmi4_f54_alpha_test(size_t count)
{
        char *buff;
        int c,timeout_count;
        int indx,indy;
        ...
        int16_t baseline_raw,counter = 0,max_rawdata,min_rawdata;
        ssize_t num_read_chars = 0,len;
        struct file *flp = NULL;
        struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;

        TP_DBG("%s: line = %d\n",__func__, __LINE__);
        // step 1 echo 1 > do_preparation
        mutex_lock(&f54->status_mutex);
        if (f54->status != STATUS_IDLE) 
        {
                if (f54->status != STATUS_BUSY) 
                {
                        TP_DBG("%s: Invalid status (%d)\n",__func__, f54->status);
                        } else {
                        TP_DBG("%s: Previous get report still ongoing\n",__func__);
                }
                mutex_unlock(&f54->status_mutex);
        ...
        if (retval < 0) 
        {
                TP_DBG("%s: Failed to do preparation\n",__func__);
                return retval;
        }
        // step 2 echo 20 > report_type
        TP_DBG("%s: line = %d\n",__func__, __LINE__);
         mutex_lock(&f54->status_mutex);
         if (f54->status != STATUS_BUSY) 
         {
         	f54->report_type = (enum f54_report_types)F54_FULL_RAW_CAP_RX_COUPLING_COMP;
         	data = (unsigned char)F54_FULL_RAW_CAP_RX_COUPLING_COMP;
         	retval = f54->fn_ptr->write(rmi4_data,
         			f54->data_base_addr,
         			&data,
         			sizeof(data));
         ...
         // step 3 echo 1 > get_report
         TP_DBG("%s: line = %d\n",__func__, __LINE__);
        command = (unsigned char)COMMAND_GET_REPORT;

	if (!is_report_type_valid(f54->report_type)) 
             {
	        TP_DBG("%s: Invalid report type\n",__func__);
	        return -EINVAL;
	}
	mutex_lock(&f54->status_mutex);
	if (f54->status != STATUS_IDLE) 
	...
	mutex_unlock(&f54->status_mutex);
	if (retval < 0) {
	        TP_DBG("%s: Failed to write get report command\n",__func__);
	        return retval;
	}
	...
	hrtimer_start(&f54->watchdog,
...
#endif
        //msleep(1000);
        // step 4: cat tp_status
        TP_DBG("%s: line = %d f54->status = %d\n",__func__,__LINE__,f54->status);
        timeout_count = 0;
        do{
           if(TX_NUM * RX_NUM *2 == f54->report_size )
             if(0 == f54->status)   
                break;
           timeout_count++;
           msleep(10);     
        ...
        if (timeout_count == FORCE_TIMEOUT_100MS) 
        {
	TP_DBG("%s: line = %d f54->report_size = %d\n",__func__,__LINE__,f54->report_size);
	goto END;
        }
        // step 5: cat report_size
        //TP_DBG("%s: line = %d f54->status = %d\n",__func__,__LINE__,f54->report_size);

        // step 6: cat report_data
        /*
        buff = f54->report_data;
        max_rawdata = min_rawdata = 0;
        TP_DBG("%s: line = %d\n",__func__, __LINE__);
        for(indx = 0; indx < f54->report_size; indx++)
        {
          //TP_DBG("%s: line = %d\n",__func__, __LINE__);
          c = *buff;
          //TP_DBG("%s: line = %d\n",__func__, __LINE__);
        ...
        */
        buff = f54->report_data;
        for(indx = 0;indx < TX_NUM; indx++) //12 23
        {
                for(indy = 0; indy < RX_NUM; indy++)
                {
                        tmp_l = *buff;
                        tmp_l = tmp_l & 0xFF;
                        //baseline_raw = c;
                        ...
                        tmp_h = *buff;
                        tmp_h = tmp_h &0xFF;
                        baseline_raw = (tmp_h << 8) | tmp_l;
                        buff++;
                        TP_DBG("baseline_raw = %d,counter=%d\n",baseline_raw,counter);
                        //max_rawdata = baseline_raw;
                        //msleep(1);  // 10
                        if((baseline_raw < Truly_array_limit[counter*2]) || (baseline_raw > Truly_array_limit[counter*2+1]))
                        {
                                TP_DBG("===>wjw_tpbaseline_fail;count[%d][%d] =%d ;Truly_array_limit[count*2]=%d,Truly_array_limit[count*2+1]=%d\n ",counter*2,(counter*2+1),baseline_raw,Truly_array_limit[counter*2],Truly_array_limit[counter*2+1]);
                                //num_read_chars += sprintf(&(buf[num_read_chars]), "0 raw data erro baseline_data[%d][%d]=%d[%d,%d]\n",indx,indy,baseline_raw,Truly_array_limit[counter*2],Truly_array_limit[counter*2+1]);
                                count = 0;
	                   goto END;
	           }
                        counter++;                        
...
#if 1
extern void synaptics_chip_reset(void);
static ssize_t gtp_sysfs_opentest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	//ret = gt9xx_open_test(i2c_client_point);
             ret = synaptics_rmi4_f54_alpha_test(len);
             TP_DBG("%s:line = %d,ret = %d\n",__func__, __LINE__,ret);
	if (ret == 1)
	{
		len = sprintf(buf, "Synaptics Test PASS \n");
	}
	else
	{
		len = sprintf(buf, "Synaptics Test FAIL \n");
	}
...
*******************************************************/
struct kobject *synaptics_debug_kobj;
s32 syna_gtp_test_sysfs_init(void)
{
    s32 ret ;
    
    synaptics_debug_kobj = kobject_create_and_add("gtp_test", NULL) ;
    //SET_INFO_LINE_INFO("Starting initializing gtp_debug_sysfs");
    if (synaptics_debug_kobj == NULL)
    {
        TP_DBG("%s: subsystem_register failed\n", __func__);
        return -ENOMEM;
    }

    ret = sysfs_create_file(synaptics_debug_kobj, &dev_attr_opentest.attr);
    if (ret)
    {
        TP_DBG("%s: sysfs_create_version_file failed\n", __func__);
        return ret;
    }

    TP_DBG("Goodix debug sysfs create success!\n");
    return 0 ;
}

void syna_gtp_test_sysfs_deinit(void)
{
    sysfs_remove_file(synaptics_debug_kobj, &dev_attr_opentest.attr);
    kobject_del(synaptics_debug_kobj);
}
...
static int synaptics_rmi4_f54_set_sysfs(void)
{
	...
	if (retval < 0) {
        ...
        /*
             retval = sysfs_create_bin_file(f54->attr_dir, &dev_alpha_test);
	if (retval < 0) {
		TP_DBG("%s: Failed to create sysfs bin file\n",	__func__);
		goto exit_2;
	}
       */
