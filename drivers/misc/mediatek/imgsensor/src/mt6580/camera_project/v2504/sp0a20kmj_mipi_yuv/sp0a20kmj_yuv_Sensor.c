...
#include "sp0a20kmjyuv_Sensor.h"
#include "sp0a20kmjyuv_Camera_Sensor_para.h"
#include "sp0a20kmjyuv_CameraCustomized.h"
...
//target
SP0A20_write_cmos_sensor(0xfd,0x01);
SP0A20_write_cmos_sensor(0xeb,0x82);//78
SP0A20_write_cmos_sensor(0xec,0x7a);//70
...
SP0A20_write_cmos_sensor(0xee,0x0c);
	...
	SP0A20_write_cmos_sensor(0xfd,0x02);
	SP0A20_write_cmos_sensor(0x26,0xab);//bf
	...
	//d65 10
	SP0A20_write_cmos_sensor(0x66,0x38);
	SP0A20_write_cmos_sensor(0x67,0x63);
	SP0A20_write_cmos_sensor(0x68,0xcd);
	SP0A20_write_cmos_sensor(0x69,0xfe);
	SP0A20_write_cmos_sensor(0x6a,0xa5);
	//indoor 11
	SP0A20_write_cmos_sensor(0x7c,0x30);
	SP0A20_write_cmos_sensor(0x7d,0x61);
	SP0A20_write_cmos_sensor(0x7e,0xf9);
	SP0A20_write_cmos_sensor(0x7f,0x1e);
	SP0A20_write_cmos_sensor(0x80,0xa6);
	//cwf   12
	SP0A20_write_cmos_sensor(0x70,0x26);
	SP0A20_write_cmos_sensor(0x71,0x4c);
	SP0A20_write_cmos_sensor(0x72,0x27);
	SP0A20_write_cmos_sensor(0x73,0x47);
	SP0A20_write_cmos_sensor(0x74,0xaa);
	//tl84  13
	SP0A20_write_cmos_sensor(0x6b,0x10);
	SP0A20_write_cmos_sensor(0x6c,0x2a);
	SP0A20_write_cmos_sensor(0x6d,0x34);
	SP0A20_write_cmos_sensor(0x6e,0x50);
	SP0A20_write_cmos_sensor(0x6f,0xaa);
	//f   14
	SP0A20_write_cmos_sensor(0x61,0xf4);
	SP0A20_write_cmos_sensor(0x62,0x0d);
	SP0A20_write_cmos_sensor(0x63,0x54);
...
SP0A20_write_cmos_sensor(0x10,0x68);//ku_outdoor
SP0A20_write_cmos_sensor(0x11,0x68);//ku_nr
SP0A20_write_cmos_sensor(0x12,0x74);//ku_dummy
SP0A20_write_cmos_sensor(0x13,0x74);//ku_low
    ...
    do {
        ...
        *sensorID = SP0A20_read_cmos_sensor(0x02);
        if (*sensorID == SP0A20_YUV_SENSOR_ID)
            break; 
        SENSORDB("Read Sensor ID Fail kmj = 0x%04x\n", *sensorID); 
        retry--; 
    } while (retry > 0);
    if (0==__gpio_get_value(CAMERA_ID))	
	*sensorID+=2;	
    if (*sensorID != SP0A20KMJ_YUV_SENSOR_ID) {
        *sensorID = 0xFFFFFFFF; 
#ifdef SLT_DEVINFO_CMM 
...
static UINT32 SP0A20Open(void)
{
	...
	for(i=0;i<3;i++)
	{
        ...
        if (0==__gpio_get_value(CAMERA_ID))
	   sensor_id+=2;
	if(sensor_id != SP0A20KMJ_YUV_SENSOR_ID)  
	{
...
SENSOR_FUNCTION_STRUCT	SensorFuncSP0A20kmjYUV=
{
	SP0A20Open,
	SP0A20GetInfo,
	SP0A20GetResolution,
