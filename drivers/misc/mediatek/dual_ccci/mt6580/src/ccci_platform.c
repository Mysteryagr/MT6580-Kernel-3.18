...
//yaohua.li start 
//extern char* Market_Area;
char buf_Market_Area[64]={0};
strncpy(buf_Market_Area,Market_Area,strlen(Market_Area));
printk("======================LGC ====find_img_to_open===========buf_Market_Area=%s===================\n",buf_Market_Area);
char modem_img_name[30];
snprintf(modem_img_name, IMG_NAME_LEN, "E%s.img",buf_Market_Area);
printk("=========LGC===1 =====buf_Market_Area=%s=====modem_img_name==%s============\n",buf_Market_Area,modem_img_name);
get_md_post_fix(md_id, buf_Market_Area, modem_img_name);
printk("=========LGC===2 =====buf_Market_Area=%s=====modem_img_name==%s============\n",buf_Market_Area,modem_img_name);
//yaohua.li end
	if (img_type == MD_INDEX) {	/*  Gen MD image name */
		snprintf(img_name[0], IMG_NAME_LEN, "modem_E%s.img",buf_Market_Area);
		snprintf(img_name[1], IMG_NAME_LEN, "modem_%s.img", buf_Market_Area);
		...
		snprintf(img_name[0], IMG_NAME_LEN, "DSP_ROM_E%s.img", buf_Market_Area);
		snprintf(img_name[1], IMG_NAME_LEN, "DSP_ROM_%s.img", buf_Market_Area);
		snprintf(img_name[2], IMG_NAME_LEN, "%s", DSP_IMAGE_NAME);
	} else {
		CCCI_MSG_INF(md_id, "ctl", "[]Invalid img type%d\n",   img_type);
	...
	for (i = 0; i < 3; i++) 
		...
		printk("======================LGC==Find img @CIP=============buf_Market_Area=%s===================\n",img_name[i]);
		...
		if (IS_ERR(filp)) 
			...
			if (i == 1) 
                        {
				snprintf(md_image_post_fix[md_id], 12, "%s",buf_Market_Area);
			printk("======================LGC==Find img @CIP=============md_image_post_fix=%s===================\n",md_image_post_fix[md_id]);
			} 
			else if (i == 0) 
			{
				snprintf(md_image_post_fix[md_id], 12, "E%s",buf_Market_Area);
		printk("======================LGC==Find img @CIP=============md_image_post_fix=%s===================\n",md_image_post_fix[md_id]);
			} 
			else 
			{
...
printk("======================LGC==Find img @CIP=============md_image_post_fix=%s===================\n",md_image_post_fix[md_id]);
	...
	for (i = 0; i < 3; i++) {
		...
		printk("======================LGC========Find img @default=======buf_Market_Area=%s===================\n",img_name[i]);
		filp = filp_open(full_path, O_RDONLY, 0644);
		if (IS_ERR(filp)) 
		{
			continue;
		...
		{	/*  Open image success */
			...
			{
				snprintf(md_image_post_fix[md_id], 12, "%s",buf_Market_Area);
printk("======================LGC=Find img @default============md_image_post_fix=%s===================\n",md_image_post_fix[md_id]);
			} 
			else if (i == 0)
		        {
				snprintf(md_image_post_fix[md_id], 12, "E%s", buf_Market_Area);
