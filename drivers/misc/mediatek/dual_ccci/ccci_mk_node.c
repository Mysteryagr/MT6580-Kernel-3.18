...
ssize_t show_attr_md1_postfix(char *buf)
{
    ...
    strncpy(buf,Market_Area,strlen(Market_Area));
    snprintf(modem_img_name, IMG_NAME_LEN, "E%s.img",buf);
    get_md_post_fix(MD_SYS1,buf, modem_img_name);
#else
	...
	CCCI_MSG("md1: %s\n", buf);
#ifdef CONFIG_WIKO_UNIFY
    printk("=====LGC=show_attr_md1_postfix==%s====\n",Market_Area);
#endif
	return strlen(buf);
}
...
ssize_t show_attr_md2_postfix(char *buf)
{
    ...
    strncpy(buf,Market_Area,strlen(Market_Area));
    snprintf(modem_img_name, IMG_NAME_LEN, "E%s.img",buf);
    get_md_post_fix(MD_SYS2,buf, modem_img_name);
#else	
    get_md_post_fix(MD_SYS2, buf, NULL);
#endif
	CCCI_MSG("md2: %s\n", buf);
#ifdef CONFIG_WIKO_UNIFY
    printk("=====LGC===%s==show_attr_md2_postfix==\n",Market_Area);
#endif
	return strlen(buf);
}
